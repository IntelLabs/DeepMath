...
from trl.import_utils import is_requests_available
...
class VLLMClient:
    ...
    def __init__(
        ...
        logger.warning("Daniel's Famous vLLM Client is used!")
        logger.info(
            f"vLLMClient arguments: base_url={base_url}, host={host}, server_port={server_port}, group_port={group_port}, connection_timeout={connection_timeout}"
        )
        ...
        self.run_hash = f"run_{str(uuid.uuid4())[:8]}"  # Unique identifier for the run
        os.makedirs(f"/tmp/{self.run_hash}", exist_ok=True)
        logger.info(f"Run cache dir: /tmp/{self.run_hash}")
    ...
    def init_communicator(self):
        ...
        # NOTE: disabling all the low-level communications
    ...
    def update_named_param(self, name: str, weights: torch.Tensor):
        ...
        weights_path = tempfile.mktemp(
            dir=f"/tmp/{self.run_hash}/", prefix="weights_", suffix=".pt"
        )

        save_file({"tensor": weights}, weights_path)

        response = self.session.post(url, json={"name": name, "path": weights_path})
        if response.status_code != 200:
            raise Exception(f"Request failed: {response.status_code}, {response.text}")

        # Broadcast the weights to the other processes
        # NOTE: no broadcast for you!
    ...
    def update_model_params(self, model: nn.Module):
        ...
        # Save the model to disk in a random tmp directory and pass the path to the server
        # Loading is done via: `model.load_state_dict(torch.load(model_path))`

        model_path = tempfile.mktemp(dir=f"/tmp/{self.run_hash}/", prefix="weights_", suffix=".pt")
        torch.save(model.state_dict(), model_path)

        url = f"{self.base_url}/update_model_params/"
        response = self.session.post(url, json={"model_path": model_path})
        if response.status_code != 200:
            raise Exception(f"Request failed: {response.status_code}, {response.text}")
    ...
    def get_config(self):
        """
        Retrieves the configuration of the vLLM server.

        Returns:
            `dict`: Configuration dictionary containing server settings.
        """
        url = f"{self.base_url}/get_config/"
